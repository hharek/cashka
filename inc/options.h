...
namespace cashka
{
	...
	class Options
	{
		private:
			...
			/* Опции по умолчанию */
			string process_name = "cashka";					/* Имя процесса */
			string version = "0.4a";						/* Версия программы */
			...
			const char * cli_getopt = "hvp:c:fH:P:";
			
			/* Опции для функции getopt_long () */
			const option cli_getopt_long[8] = 
			{
			...
			/* Опции в конфиге */
			string config_process_name;						/* Имя процесса, заданный в конфигурационному файле как "process_name" */
			string config_pid_file;							/* Путь к PID-файлу, заданный в конфигурационном файла как "pid_file" */
			...
			bool config_foreground = false;					/* Запускать ли на переднем плане (не в фоне), заданный в конфигурационном файле */
		...
		private:
			...
			void config_check_options (nlohmann::json & process_name);
			void config_process_name_set (nlohmann::json & process_name);
			...
			void check_process_name (const char * process_name);
